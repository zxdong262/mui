...
$(function ($) {
    ...
    //h3, h2
    $('#content > .html-output > h3, #content > h3').addClass('sub-title');
    ...
    //nav render
    var ht = ''
    ,res = $('#content > h2, #content > .html-output > h3, #content > h3')
    ,len = res.length
    ,emp = $('not-now')
    ...
    $('.qr').each(function() {
        var t = $(this)
        t.qrcode({width: 128,height: 128,text: t.text() })
    })
    ...
    res.each(function (index) {
        var t = $(this),
            ...
            next = res.eq(index + 1),
            navstr = '#nav-' + index
            tops.push(t.offset().top + offsetFix*index)
    ...
    debug('isTouchDevice: '+mui.isTouchDevice)
    $('#wrapper').addClass('body')
    if(mui.scrolls.length) mui.refreshScrolls()

    $('#wrapper').after('<div id="nav"><div id="nav-inner"><ul class="nav-ul">' + ht + '</ul></div></div><a class="bg-fff label border corner nav-open" id="nav-toggle" href="javascript:;"><span class="open-txt">Hide &raquo;</span><span class="close-txt">&laquo; Show</span></a>')
...
function autoWidth() {
    ...
    var w = $(window).width()
    if(w > 500 + 30 * 2 + 200) {
        $('#content').attr('class', '').addClass('m270')
        $('body').removeClass('hide-nav')
        $('#nav-toggle').removeClass('nav-close').addClass('nav-open')
    } else if(w > 560) {
        $('#content').attr('class', '').addClass('m30')
    } else if(w > 380) {
        $('#content').attr('class', '').addClass('m10')
    } else {
        $('#content').attr('class', '')
    }
}
...
function updateTops() {
    window.tops = [0]
    $('#content > h2, #content > .html-output > h3, #content > h3').each(function (index) {
        var t = $(this)
        tops.push(t.offset().top + offsetFix)
    })
    tops.push($('#content').height())
}
...
function setNavHeight() {
    var w = $('#nav').height()
    if($('#nav ul').height() + 32 > w) $('#nav-inner').addClass('scroll')
    else $('#nav-inner').prop('scrollTop', 0).removeClass('scroll')
}

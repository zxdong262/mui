...
window.mui = {
    ...
    ,refreshScrolls : function() {
        $.each(this.scrolls, function(i,t) {
            t.refresh()
        })
    }
	//check if the device is supporting position fixed, for now assume adroid 4.0+ is good
	,good: navigator.userAgent.match( /Android [^123]/ )
    ,UITimer : null
    ,inited : false
...
mui.initUI = function(fa) {
    ...
    //do once
    if(!mui.inited) initOnce()
    function initOnce() {
        mui.inited = true
		var b = $('.body')
		,cls1 = mui.isTouchDevice?'touch-device' : 'not-touch-device'
		,cls2 = mui.good?'good-device' : ''
		$('body').addClass(cls1 + ' ' + cls2)
		if(b.length) {
			var f = $('.footer')
			,h = $('.header')
			,hasHead = !!h.length
			,hasFoot = !!f.length
			if(hasFoot) b.css('bottom', f.height())
			else b.after('<div class="footer"></div>')
			if(hasHead) b.css('top', h.height())
			else b.before('<div class="header"></div>')
        ...
        if(!mui.good && mui.isTouchDevice && b.length) {
            if(!b.prop('id')) b.prop('id','id-body')
            window.document.addEventListener('touchmove', function(e) {
                e.preventDefault();
            }, false);
            var bs = new iScroll(b.prop('id'), {
                vScrollbar:false
                ,hScroll:false
...
mui.popup = function (opts) {
    ...
    $('#popup').animate({
        ...
        if(defs.timer) mui.UITimer = setTimeout(function() {
            popout()
        },parseInt(defs.timer))
    })
}
